...
The assumption that the sample is neither time-stretched nor pitch-shifted is not true for the majority of cases. In the dataset we provide, 57.5\% of samples are pitch-shifted. Pitch-shifting is often required for the sample to match the tonality of the song, and time-stretching is often required to adjust for tempo differences between sample and song.
...
Thus, we perform one partially-fixed NMF for each pitch-shift in $P$ and obtain the activations $H_{\mathrm{s}}^{\mathrm{p}}$ corresponding to the pitch-shifted templates$W_{\mathrm{o}}^{\mathrm{p}}$.
...
%
In such a scenario, DTW can be used to align the activations $H_\mathrm{o}$ with the activations $H_{\mathrm{s}}^{\mathrm{p}}$ in the song for each pitch-shift factor $\mathrm{p}$. A distance matrix $D$ is constructed using the pair-wise correlation between the $K$ dimensional activations. The dimensions of $D$ is $N_\mathrm{o}\times N_\mathrm{s}$, where $N_\mathrm{o}$ is the number of frames in the original sample activations and $N_\mathrm{s}$ is the number of frames in the song activations.
\begin{figure}[t]
\centering
...
Such a scheme is applied when only one global optimal path that aligns the two entire time-series is required. In the case of sample detection, a sample could be present in multiple locations within the song and hence, we ideally require alignment paths at multiple locations in $D$ that align the entire sample with the segments of the song. Therefore, we modify the initialization of $C$ by only accumulating the distances along the dimension of the sample, which in our case is the column. This subsequence DTW scheme\cite{Müller2007} initializes the cost matrix $C$ as follows:
\begin{equation}
\label{cost_init_new}
\begin{aligned}
C(1,j) &= D(1,j)\\
...
This normalization is applied to all sets of activations, namely: sample activations $H_\mathrm{o}$ and the song activations $H_{\mathrm{s}}^{\mathrm{p}}$ for each $p \in P$
...
The distance matrix $D$ and properties of the extracted path can be used to indicate the presence of a sample in the query song. \figref{fig3} shows an example of a distance matrix with a looped sample. The low cost parallel blue paths indicate the presence of the sample. The last row of the cost matrix $C$, containing the alignment path costs normalized by the length of the path, will be referred to as \textit{DTW cost function}. Local minima in the DTW cost function indicate potential sample end points.
...
Overall, the following features are extracted for every unique start location: (Note that when we refer to end points here, we refer to the end points that map to this unique start location)
\begin{inparaenum}[(i)]
...
\begin{inparaenum}[(i)]
    \item   Primarily, the artists were sorted in decreasing order of number of songs sampled. Artists like James Brown, Stevie Wonder, Michael Jackson, Queen were among the artists whose original songs were chosen.
    \item   Good distribution among genres was striven for. Samples range from Pop, Rock, Funk, and Hip-Hop genres.
		\item		In addition, some of the samples were picked based on the popularity of the songs that are sampling according to the users of Whosampled.
\end{inparaenum}
...
\begin{inparaenum}[(i)]
    ...
    \item   pitch shift(in semi-tone) of the sample in the sampling song. 57.5\% of the samples are pitch-shifted.
\end{inparaenum}
...
More specifically, the start points associated with minimum DTW cost within a 1 second window of the ground truth annotation are labeled `1' and the rest are labeled `0'. Multiple start points within the tolerance window are processed in a way that only the minimum are chosen and the rest are removed.%In some instances there were multiple start points within the tolerance window. To break these ties, instead of choosing the closest start location, the location that had the minimum cost DTW path was labeled `1' and the rest are labeled `0'. The reason for doing this is that, upon observation, it was found that some of these candidates were false positives with high DTW costs and we would like to train the classifier with less noisy data.
...
If multiple positive detections are obtained in the tolerance window, they are grouped as one true positive. %{\color{red}wait. is this necessary? can't we just merge them and count them as one TP???}
...
The \tabref{results} report the training accuracy as well as the testing accuracy for our classifier. Note that we compute the metrics using the probability threshold for classification as 0.5.
...
The results in \tabref{results} show that the presented method is somewhat effective at detecting the presence of sampling in a set of songs with a very low false-positive rate and a reasonably high precision. The low recall of the method can be attributed to the highly imbalanced nature of the problem: the testing dataset has 289 instances of sampling against around 74,000 instances that aren't locations of sampling. The training set is similarly skewed in its distribution of positive and negative classes. %We observed that recall could be improved by balancing the training data by undersampling the negative class instances but it led to a drastic decrease in the precision and f-measure. 

Another possible reason for the low recall is that the method isn't always accurate when it comes to picking candidates. More specifically, it misses approximately 7\% of sampling instances during the feature extraction stage. This number is computed by setting the probability threshold for classification to 0, essentially classifying all instances as a sample instances. A closer investigation of some training samples showed that sometimes the DTW cost function did not have a minimum at respective sample locations. In these cases, the distance matrix would not contain clear alignment paths. {\color{red} Visualization for this?} %This could lead to the problematic cases being misclassified as negatives, resulting in a low recall.
%
