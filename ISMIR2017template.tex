...
\label{class}
Each unique start location is represented by an 13-dimensional feature vector, which is a data point that can be used as the input to a binary classifier for detecting whether a sample is present or not.
...
Eighty popular samples (according to the users of Whosampled) were selected from the catalog of songs by popular and frequently sampled artists such as James Brown, Stevie Wonder, Michael Jackson, and Queen. The resulting set has a balanced distribution among the genres Pop, Rock, Funk and Hip-Hop.
    ...
    %The songs that used these samples are in most cases from the genres Hip-Hop, Pop, and Rap. 
...
In the context of plagiarism detection, a high precision enables such a system to be used with high confidence in case of a positive detection of plagiarism. However, a low recall system ``favors'' the sampling artists so the involvement of human experts remains a requirement.% {\color{red} Add something about the benefit of high precision in plagiarism detection?}
